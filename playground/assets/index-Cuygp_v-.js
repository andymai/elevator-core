...
)`},P=13,Ft=[{name:"Overnight",durationSec:45,ridersPerMin:6,originWeights:y(P),destWeights:y(P)},{name:"Morning rush",durationSec:75,ridersPerMin:48,originWeights:[14,...Array.from({length:P-1},()=>.25)],destWeights:[0,...H(P-1)]},{name:"Midday interfloor",durationSec:60,ridersPerMin:30,originWeights:y(P),destWeights:y(P)},{name:"Lunchtime",durationSec:45,ridersPerMin:42,originWeights:Array.from({length:P},(e,t)=>t===6?3:1),destWeights:Array.from({length:P},(e,t)=>t===6?4:1)},{name:"Evening exodus",durationSec:75,ridersPerMin:42,originWeights:[0,...H(P-1)],destWeights:[14,...Array.from({length:P-1},()=>.25)]}],Ot={id:"skyscraper-sky-lobby",label:"Skyscraper (sky lobby)",description:"Twelve floors, three cars. Morning rush saturates two cars fast — the third's full-load bypass (80 %/50 %) stops it from detouring for upward hall calls it can't serve.",defaultStrategy:"etd",phases:Ft,seedSpawns:0,abandonAfterSec:180,hook:{kind:"bypass_narration"},featureHint:"Direction-dependent bypass (80 % up / 50 % down) on all three cars — baked into the RON below. Watch the fullest car skip hall calls.",ron:`SimConfig(
    building: BuildingConfig(
