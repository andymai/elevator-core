...
)`},x=13,Ft=[{name:"Overnight",durationSec:45,ridersPerMin:6,originWeights:S(x),destWeights:S(x)},{name:"Morning rush",durationSec:75,ridersPerMin:180,originWeights:[14,...Array.from({length:x-1},()=>.25)],destWeights:[0,...D(x-1)]},{name:"Midday interfloor",durationSec:60,ridersPerMin:60,originWeights:S(x),destWeights:S(x)},{name:"Lunchtime",durationSec:45,ridersPerMin:140,originWeights:Array.from({length:x},(e,t)=>t===6?3:1),destWeights:Array.from({length:x},(e,t)=>t===6?4:1)},{name:"Evening exodus",durationSec:75,ridersPerMin:170,originWeights:[0,...D(x-1)],destWeights:[14,...Array.from({length:x-1},()=>.25)]}],Ot={id:"skyscraper-sky-lobby",label:"Skyscraper (sky lobby)",description:"Twelve floors, three cars. Morning rush saturates two cars fast — the third's full-load bypass (80 %/50 %) stops it from detouring for upward hall calls it can't serve.",defaultStrategy:"etd",phases:Ft,seedSpawns:0,hook:{kind:"bypass_narration"},featureHint:"Direction-dependent bypass (80 % up / 50 % down) on all three cars — baked into the RON below. Watch the fullest car skip hall calls.",ron:`SimConfig(
    building: BuildingConfig(
...
)`},I=8,Ht=[{name:"Overnight",durationSec:60,ridersPerMin:4,originWeights:S(I),destWeights:S(I)},{name:"Morning exodus",durationSec:75,ridersPerMin:90,originWeights:[0,...D(I-1)],destWeights:j(I)},{name:"Midday quiet",durationSec:60,ridersPerMin:12,originWeights:S(I),destWeights:S(I)},{name:"Afternoon drift",durationSec:45,ridersPerMin:30,originWeights:Array.from({length:I},(e,t)=>t===0?3:1),destWeights:S(I)},{name:"Evening return",durationSec:60,ridersPerMin:75,originWeights:j(I),destWeights:[0,...D(I-1)]}],Dt={id:"residential-tower",label:"Residential tower",description:"Eight floors, two cars. Asymmetric day: morning exodus, quiet midday, evening return. Predictive parking anticipates the next peak by the arrival-log rate signal.",defaultStrategy:"etd",phases:Ht,seedSpawns:0,hook:{kind:"predictive_parking",windowTicks:9e3},featureHint:"Predictive parking with a 2.5-min window — during the midday slump, idle cars pre-position toward the floors that spiked most recently.",ron:`SimConfig(
    building: BuildingConfig(
...
)`},W=10,$t=[{name:"Overnight",durationSec:45,ridersPerMin:4,originWeights:S(W),destWeights:S(W)},{name:"Check-out rush",durationSec:60,ridersPerMin:55,originWeights:[0,.5,...Array.from({length:W-2},()=>1)],destWeights:[5,2,...Array.from({length:W-2},()=>.3)]},{name:"Daytime",durationSec:75,ridersPerMin:25,originWeights:S(W),destWeights:S(W)},{name:"Check-in rush",durationSec:60,ridersPerMin:50,originWeights:[4,1,...Array.from({length:W-2},()=>.3)],destWeights:[0,.5,...Array.from({length:W-2},()=>1)]},{name:"Late night",durationSec:60,ridersPerMin:14,originWeights:[2,2,...Array.from({length:W-2},()=>.5)],destWeights:[.5,.5,...Array.from({length:W-2},()=>1)]}],Nt={id:"hotel-24-7",label:"Hotel 24/7",description:"Ten floors, three cars in DCS mode. Low-rate baseline with check-in / check-out bumps; deferred commitment reallocates assignments while cars are still far from the rider's origin.",defaultStrategy:"destination",phases:$t,seedSpawns:0,hook:{kind:"deferred_dcs",commitmentWindowTicks:180},featureHint:"Deferred DCS with a 3-s (180-tick) commitment window — sticky assignments keep re-competing until a car is close to the rider, yielding better matches under bursty demand.",ron:`SimConfig(
    building: BuildingConfig(
        name: "Hotel 24/7",
        stops: [
            StopConfig(id: StopId(0), name: "Lobby",      position: 0.0),
...
)`},H=5,Xt=[{name:"Keynote lets out",durationSec:45,ridersPerMin:240,originWeights:Array.from({length:H},(e,t)=>t===H-1?8:1),destWeights:[5,2,1,1,0]},{name:"Tapering",durationSec:90,ridersPerMin:40,originWeights:S(H),destWeights:S(H)},{name:"Between sessions",durationSec:135,ridersPerMin:8,originWeights:S(H),destWeights:S(H)}],Yt={id:"convention-burst",label:"Convention burst",description:"Five-floor convention center. A keynote ends and 200+ riders spill into the elevators at once — an acute stress test rather than a day cycle.",defaultStrategy:"etd",phases:Xt,seedSpawns:120,hook:{kind:"arrival_log"},featureHint:"Arrival-rate signal lights up as the burst hits — `DispatchManifest::arrivals_at` feeds downstream strategies the per-stop intensity for the next 5 minutes.",ron:`SimConfig(
    building: BuildingConfig(
