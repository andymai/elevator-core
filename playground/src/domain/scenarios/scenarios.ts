...
// ─── Airport people mover — dual counter-rotating loops ─────────────
//
// Two LineKind::Loop lines sharing seven station names (Terminal +
// Concourse A..F) but distinct StopIds. The inner loop's stop
// positions are mirrored (circumference - outer_pos) so the forward
// sweep traverses concourses in reverse physical order — the encoding
// trick that gives counter-rotation when both loops' cars travel
// "forward" in their own frame. Headway is enforced by loop_lines, so
// trains can't visually overlap (this addresses the constraint that
// caused the prior pedway scenario, PR #797, to be reverted).
//
// 4 cars, 2 per loop, LoopSchedule dispatch (fixed-headway timetable).
// Weight vectors are 14 entries long, indexed by stops[] order:
//   0..6  outer-loop platforms (Terminal, A, B, C, D, E, F)
//   7..13 inner-loop platforms (Terminal, F, E, D, C, B, A)
// Cross-loop spawns (outer-T → inner-F) have no route through the
// topology graph and are effectively dropped; the same-loop majority
// drives the demo, so the weights stay symmetric for simplicity.
...
const airport: ScenarioMeta = {
  ...
  // Placeholder strategy name — the RON's per-group LoopSchedule is
  // what actually runs the dispatch. The strategy popover is hidden
  // when an airport-mode scenario is active so this never reaches the
  // wasm side via setStrategy.
  defaultStrategy: "scan",
  ...
  buildingName: "Airport People Mover",
  stops: AIRPORT_STOPS,
  // 4 cars fixed by the dual-loop structure (2 per loop). The tweak
  // drawer locks `cars` so the slider doesn't try to regenerate a
  // multi-line config the same way it does for flat scenarios.
  defaultCars: 4,
  elevatorDefaults: {
    maxSpeed: 14.0,
    acceleration: 1.0,
...
// elevator second sets up the "zoom out" jump in scale. Airport sits
// in the middle of the lineup as the only horizontal-topology
// scenario, giving variety between the two vertical scenarios.
// Convention center last — it's an acute stress-test scenario rather
// than a typical day cycle, useful but niche, so it sits at the end
// of the row instead of greeting cold visitors.
