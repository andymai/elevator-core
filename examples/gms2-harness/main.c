...
/* ── GMS return-register bridge shims (issue #876) ───────────────────
 * Forward-declared here because cbindgen does not expand the
 * `gms_shim!()` macro in crates/elevator-ffi/src/gms_shims.rs, so
 * these companion symbols aren't reflected in elevator_ffi.h. Each
 * declares the same arguments as the original but returns `double`
 * with the bit pattern of the original `u32` / `EvStatus` / `*mut
 * EvSim` etc. cast through `u64`. */
extern double ev_abi_version_gms(void);
extern double ev_sim_step_gms(struct EvSim *handle);
extern double ev_sim_create_gms(const char *config_path);
extern double ev_sim_hall_call_count_gms(struct EvSim *handle);
...
int main(int argc, char **argv) {
    ...
    /* ── Return-register bridge validation (issue #876) ─────────────
     * Each shimmed call returns a `double` whose bit pattern is the
     * underlying integer / enum / pointer cast as `u64`. This block
     * exercises the Rust-side ABI for the shim layer; the GMS-side
     * `external_call` dispatcher can't be reached from C, but the
     * Rust convention (which is where the bug actually lived) is
     * covered here. */
    {
        ...
        double step_real = ev_sim_step_gms(roundtrip);
        uint64_t step_bits;
        memcpy(&step_bits, &step_real, sizeof(step_bits));
        if ((uint32_t)step_bits != (uint32_t)EvStatus_Ok) {
            fprintf(stderr,
                    "FAIL: ev_sim_step_gms bit pattern was %llu, expected %u\n",
                    (unsigned long long)step_bits, (unsigned)EvStatus_Ok);
            ev_sim_destroy(roundtrip);
            return 1;
        }

        double count_real = ev_sim_hall_call_count_gms(roundtrip);
        uint64_t count_bits;
        ...
        printf("GMS shim bit-pattern bridge: abi=%u step=Ok hall_calls=%u\n",
               (uint32_t)abi_bits, count_direct);
    }

    /* Pointer-return shim: round-trip a new sim handle through the
     * shim's `double` encoding to confirm pointers survive. This
     * mirrors the existing handle round-trip above but flowing
     * through the shim's f64 path. */
    {
